<template>
              ...
              <DataTable :value="activeTournaments" :paginator="true" :rows="5">
                ...
                <Column header="Action">
                  <template #body="slotProps">
                    ...
                    <Button label="Quit" size="small" severity="danger"
                      @click="handleQuit(slotProps.data.id)" />
                  </template>
                </Column>
              </DataTable>
            </template>
  ...
  const fetchUserProfile = async () => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL_USERS}/profile`, {
        uuid: '3ee9b572-79b0-11ef-b4bc-0242ac110002' // Replace with actual user UUID
      });
  ...
  const fetchPlayerTournaments = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL_PUBLIC_USER}/tournaments/player/${userData.value.UUID}`);
